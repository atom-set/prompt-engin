...
description: 文档格式规范，包括任务清单、测试用例、文章报告等格式要求。当用户需要创建文档时，自动应用此规范。
...
当用户需要：
- **创建任务清单**时，自动应用此规范（表格格式、任务进度表格等）
- **编写测试用例**时，自动应用此规范（表格格式、测试步骤等）
- **生成报告文档**时，自动应用此规范（总分总结构、时间格式等）
- **创建 WIKI 文档**时，自动应用此规范（文档结构、格式要求等）
...
- **与 `time-format` 技能配合**：确保文档中的时间格式正确（使用 `openskills read time-format` 加载）
- **与核心规则配合**：确保代码示例格式正确（核心规则，始终加载）
- **与 `wiki-output` 技能配合**：WIKI 文档时同时应用 WIKI 输出规范（使用 `openskills read wiki-output` 加载）
