---
name: time-format
description: 时间格式规范，强制要求所有时间字段都必须通过工具动态获取，严禁使用任何假设时间。当用户需要处理时间相关的内容时，自动应用此规范。
tags: [rules, prompt-engine, time, format]
---
...
当用户需要：
- **创建包含时间的文档**时，自动应用此规范（必须使用 `date` 命令获取时间）
- **处理时间字段**时，自动应用此规范（禁止使用假设日期）
- **生成报告或文档**时，自动应用此规范（确保时间格式正确）
...
- **与 `document-format` 技能配合**：确保文档格式和时间格式都正确（使用 `openskills read document-format` 加载）
- **与核心规则配合**：确保代码中的时间处理正确（核心规则，始终加载）
