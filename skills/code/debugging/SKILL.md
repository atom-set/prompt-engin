...
- **依赖关系**：
  - `error-handling-strategy`：定位问题后，如果需要添加错误处理代码（如 try-catch、错误日志等），应遵循错误处理规范，确保错误处理机制统一、完整
  - `error-logging`：定位问题过程中，如果需要记录错误日志，应遵循错误日志记录规范
  - `error-message-format`：定位问题后，如果需要设计错误信息，应遵循错误信息格式规范
- 与核心规则配合使用
