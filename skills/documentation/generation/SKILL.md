...
description: 文档生成规范，整合技术方案、架构图、WIKI 等文档类型的规范
priority: 4
...
- **依赖关系**：
  - `wiki-output`：生成 WIKI 文档时，必须遵循 WIKI 文档规范，包括总分总结构、Mermaid 图表转换等
  - `document-format`：生成文档时，必须遵循文档格式规范，确保文档结构清晰、格式统一
  - `architecture-diagram-template`：包含架构图时，必须遵循架构图文档规范，包括图表模块化、说明可折叠等
- 与核心规则配合使用
...
- **工具位置**：`.claude/skills/documentation/wiki-output/scripts/mermaid_to_png.sh`
...
- **工具位置**：`.claude/skills/documentation/wiki-output/scripts/extract_and_convert_mermaid.py`
