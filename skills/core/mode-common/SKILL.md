---
name: mode-common
description: 模式通用规则，包括模式切换、响应格式等。本规范定义了 Plan 和 Act 两种模式下的通用规则，包括模式切换条件、响应格式规范、执行顺序要求等核心机制，确保系统在不同模式下的行为一致性和可预测性。
priority: 1
...
- **依赖关系**：
  - `tool-permission-system`：模式切换时，必须执行工具调用前的统一检查机制，该机制包含模式检查、工具类型检查和方案完整性检查
  - `plan-mode`：Plan 模式下的行为规范，包括允许和禁止的操作、方案输出要求等
  - `act-mode`：Act 模式下的行为规范，包括执行确认、执行流程等
- 与核心规则配合使用
...
**核心原则**：首次响应（对话开始后的第一次响应）必须严格遵守以下特殊要求，确保规范从一开始就被正确执行。
...
2. **需求类型识别**：
   - 在输出模式标识后，立即识别用户需求类型
   - 如果识别为"代码修改需求"，必须先输出完整方案，再调用任何工具
   - 如果识别为"查询需求"，可以调用只读工具

3. **代码修改需求的识别标准**：
   - 用户输入包含以下关键词，应视为"代码修改需求"：
     - **动作词**："修改"、"修复"、"添加"、"删除"、"实现"、"创建"、"更新"、"调整"、"优化"、"重构"、"改进"
     - **问题描述**："bug"、"错误"、"问题"、"漏洞"、"需要"、"应该"
   - 如果识别为代码修改需求，必须：
     1. 输出模式标识
     2. 输出完整方案（5个部分）
     3. 输出更新后的计划
     4. 等待用户输入"Act"
     5. 禁止在输出方案前调用任何工具（包括只读工具）
...
AI（首次响应）：
  ...
  [识别为代码修改需求]
  [输出完整方案：需求分析、修改方案、影响分析、实施计划、风险评估]
  [输出更新后的计划]
  [提醒用户输入"Act"]
...
用户：修改 sync.sh 脚本
AI（首次响应）：
  [直接调用 read_file 读取文件]  ❌ 错误：未先输出模式标识和方案
  [调用 codebase_search 搜索代码]  ❌ 错误：未先输出模式标识和方案
```
