...
description: 安全规则和权限规则，系统化整理权限矩阵。本规范提供了 Plan 和 Act 两种模式下的权限规则快速参考，包括工具权限分类、行为权限分类、安全检查机制等，确保所有操作都以安全为前提，权限明确、确认优先。
priority: 1
...
本规范提供了 Plan 和 Act 两种模式下的权限规则快速参考和系统化整理，包括工具权限分类、行为权限分类、安全检查机制、权限升级机制等核心内容。本规范确保所有操作都以安全为前提，权限明确、确认优先，是其他详细规则文件的整合和索引。详细的行为规范请参考对应的规则文件（如 `plan-mode.md`、`act-mode.md`、`tool-permission-system.md` 等）。
...
**场景**：Plan 模式下，用户要求修改代码

**执行流程**：
```
1. 用户要求修改代码
2. 分析需求，输出完整方案（5个部分）
3. 提醒："当前处于Plan模式，如需执行修改，请输入'Act'切换到执行模式"
4. 禁止调用 search_replace、write 等修改工具
5. 等待用户输入 "Act" 指令
```

### 示例2：Act 模式下执行修改

**场景**：用户已输入 "Act" 指令，方案完整
...
3. 切换到 Act 模式
4. 简要确认计划
5. 执行修改操作（调用 search_replace、write 等工具）
6. 执行完成后，自动返回 Plan 模式
```
