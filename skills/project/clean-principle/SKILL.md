---
name: project-clean-principle
description: 项目清洁原则，避免将 AI 辅助开发工具和非业务相关的脚本混入项目核心代码
priority: 4
tags: ['project', 'clean', 'principle']
---
...
**场景**：需要创建一个将 Mermaid 图表转换为 PNG 的工具

**正确做法**：
```bash
# ✅ 正确：放在 .cursor-rules/utils/ 目录
mkdir -p .cursor-rules/utils
touch .cursor-rules/utils/mermaid_to_png.sh

# ❌ 错误：放在项目 scripts/ 目录
# mkdir -p scripts/utils
# touch scripts/utils/mermaid_to_png.sh
```

**工具说明**：
在 `.cursor-rules/utils/README.md` 中添加说明：
```markdown
## mermaid_to_png.sh

将 Mermaid 流程图代码转换为 PNG 图片文件。

**使用方法**：
```bash
bash .cursor-rules/utils/mermaid_to_png.sh input.mmd
```
...
**场景**：使用 Mermaid 转换工具生成图片后，需要存放图片

**正确做法**：
```bash
# ✅ 正确：放在 .cursor-rules/assets/ 目录
mkdir -p .cursor-rules/assets/project-name
# 生成的图片存放在 .cursor-rules/assets/project-name/架构图.png

# ❌ 错误：放在项目根目录的 assets/ 目录
# mkdir -p assets
# # 生成的图片存放在 assets/架构图.png
```

**文档引用**：
在项目文档中使用相对路径引用：
```markdown
![架构图](../.cursor-rules/assets/project-name/架构图.png)
```

#### 示例3：清理项目污染

**场景**：发现项目的 `scripts/` 目录中有 AI 辅助工具

**清理流程**：
```bash
# 1. 识别污染内容
ls scripts/utils/  # 发现 ai_tool.sh
...
# 4. 移动工具生成的产物（如果有）
mv assets/ai-generated .cursor-rules/assets/ 2>/dev/null || true

# 5. 清理空目录
rmdir scripts/utils 2>/dev/null || true

# 6. 更新 .gitignore（如果不提交）
echo ".cursor-rules/" >> .gitignore
```
